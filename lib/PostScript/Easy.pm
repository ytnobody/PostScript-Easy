package PostScript::Easy;

use strict;
use vars qw($VERSION @ISA @EXPORT);
use Carp;
use Exporter;
use PostScript::Easy::EPS;
...
PostScript::Easy was forked from L<PostScript::Simple> v0.07. 
...
# Comments Section
  ...
  push @$page, "\%\%Creator: PostScript::Easy perl module version $VERSION\n";
...
# Prolog Section
  ...
  push @$page, "\%\%BeginResource: PostScript::Easy\n";
...
sub get# {{{
{
  ...
  $page = _builddocument($self, "PostScript::Easy generated page");
  $doc = "";
  foreach $i (@$page) {
    if (ref($i) eq "SCALAR") {
      $doc .= $$i;
...
sub geteps# {{{
{
  ...
  $eps = new PostScript::Easy::EPS(source => $self->get);
  return $eps;
}# }}}
...
listed at the top of the PostScript::Easy module in the C<%pscolours> hash.
...
sub importepsfile# {{{
{
  ...
  $eps = new PostScript::Easy::EPS(file => $file);
...
sub _add_eps# {{{
{
  ...
  if (ref($_[0]) ne 'PostScript::Easy::EPS') {
...
The Original L<PostScript::Simple> module was created by Matthew Newton, with ideas
