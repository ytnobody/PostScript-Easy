...
PostScript::Easy::EPS - EPS support for PostScript::Easy
...
PostScript::Easy::EPS was forked from L<PostScript::Simple::EPS> v0.01. 
And, basic usage is the same to PostScript::Simple::EPS.

PostScript::Easy::EPS allows you to add EPS files into PostScript::Easy
objects.  Included EPS files can be scaled and rotated, and placed anywhere
inside a PostScript::Easy page.
...
This module requires C<PostScript::Easy>, C<strict>, C<Carp> and C<Exporter>.
...
Create a new PostScript::Easy::EPS object. The options
that can be set are:
...
Example:
    ...
    $ps = new PostScript::Easy(landscape => 1,
                                 eps => 0,
                                 xsize => 4,
                                 ysize => 3,
                                 units => "in");

    $eps = new PostScript::Easy::EPS(file => "test.eps");
...
#  $$self{"epsfile"} .= "\%\%EndDocument\n";
  ...
  if (($$self{bbx2} - $$self{bbx1} == 0) ||
    ...
    $self->_error("PostScript::Easy::EPS: Bounding Box has zero dimension");
    return 0;
  }
...
command to insert it each time it is used. C<object> is a PostScript::Easy
...
sub preload# {{{
{
  ...
  croak "no PostScript::Easy module provided" if (!defined $ps);
...
Satoshi Azuma L<ytnobody at gmail dot com>

The Original L<PostScript::Simple::EPS> module was written by Matthew Newton, 
after prods for such a feature from several people around the world. A useful 
importeps function that provides scaling and aspect ratio operations was 
gratefully received from Glen Harris, and merged into this module.
